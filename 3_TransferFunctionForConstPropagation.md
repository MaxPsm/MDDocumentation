...
![Значение переменной и оператор сбора на полурешётке](task2/v1.JPG)
...
![Область определения потока данных и значений потока данных](task2/v2.JPG)

Теперь определим оператор сбора (передаточную функцию) для случая нескольких переменных:

![Передаточная функция в задаче о распространении констант](task2/v3.JPG)
